    ...
    <div class="mx-auto max-w-[1320px] px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
        ...
        >
            <div class="flex items-start justify-between gap-4">
                <div class="flex items-start gap-4">
                    <div
                        class="inline-flex h-12 w-12 items-center justify-center rounded-2xl bg-orange-600 text-white"
                    >
                        <svg
                            viewBox="0 0 24 24"
                            class="h-6 w-6"
        ...
        >
            ...
            >
                <div class="flex items-center justify-between">
                    <div
                        class="inline-flex h-11 w-11 items-center justify-center rounded-xl bg-emerald-100 text-emerald-600"
                    >
                        <svg
                            viewBox="0 0 24 24"
                            class="h-5 w-5"
            ...
            >
                <div class="flex items-center justify-between">
                    <div
                        class="inline-flex h-11 w-11 items-center justify-center rounded-xl bg-indigo-100 text-indigo-600"
                    >
                        <svg
                            viewBox="0 0 24 24"
                            class="h-5 w-5"
                        ...
                        >
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    <div
                                        class="inline-flex h-10 w-10 items-center justify-center rounded-full text-sm font-bold text-white bg-emerald-500"
                                    >
                            ...
                            <td class="px-6 py-4">
                                <div>
                                    <span
                                        class="inline-flex items-center rounded-full px-3 py-1 text-sm font-semibold bg-emerald-100 text-emerald-700"
                                    >
                                        {{ utilisateur.type }}
                                    </span>
                                </div>
                            ...
                            <td class="px-6 py-4">
                                ...
                                >
                                    ...
                                    <button
                                        ...
                                        @click="ouvrirEdition(utilisateur)"
                                    >
                                        <svg
                                            viewBox="0 0 24 24"
                                            class="h-5 w-5"
                                        ...
                                        >
                                            <path d="M12 20h9" />
                                            <path
                                                d="m16.5 3.5 4 4L8 20l-5 1 1-5z"
                                            />
                                        </svg>
                                    </button>
...
const statistiques = computed(() => {
    ...
    const patients = utilisateurs.value.filter(
        (item) => item.type === "Patient",
    ).length;
    const actifs = utilisateurs.value.filter(
...
const utilisateursFiltres = computed(() => {
    ...
    return utilisateurs.value.filter((item) => {
        ...
        const correspondStatut =
            filtreStatut.value === "Tous" || item.statut === filtreStatut.value;
...
async function confirmerSuppression() {
    try {
        messageErreur.value = "";
        await supprimerUtilisateurAdministrateur(
            idUtilisateurSuppression.value,
        );
        await chargerUtilisateursAdministrateur();
        fermerSuppression();
    } catch (error) {
...
async function basculerStatutUtilisateur(utilisateur) {
    ...
    try {
        ...
        messageErreur.value =
            error?.response?.data?.message ||
            "Impossible de changer le statut de cet utilisateur pour le moment.";
    }
}
...
async function chargerUtilisateursAdministrateur() {
    ...
    try {
        ...
        messageErreur.value =
            error?.response?.data?.message ||
            "Impossible de charger les utilisateurs administrateur.";
    } finally {
