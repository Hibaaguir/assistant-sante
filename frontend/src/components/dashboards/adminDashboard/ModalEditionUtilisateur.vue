<template>
    <Teleport to="body">
        <div
            v-if="ouvert"
            class="fixed inset-0 z-[1400] flex items-center justify-center bg-slate-900/45 p-4"
        >
            <div
                class="w-full max-w-[500px] rounded-2xl bg-white p-6 shadow-2xl"
            >
                <div class="mb-5 flex items-start justify-between">
                    <div>
                        <h3
                            class="text-[32px] font-semibold leading-none text-slate-900"
                        >
                            Modifier un utilisateur
                        </h3>
                        <p class="mt-2 text-sm text-slate-500">
                            Modifiez les informations d'un utilisateur existant.
                        </p>
                    </div>
                    <button
                        type="button"
                ...
                <form class="space-y-4" @submit.prevent="$emit('enregistrer')">
                    ...
                    <div class="flex items-center justify-end gap-3 pt-3">
                        <button
                            type="button"
                            class="rounded-2xl bg-slate-100 px-6 py-2.5 text-xl font-semibold text-slate-600"
                            @click="$emit('fermer')"
                        >
                            Annuler
                        </button>
                        <button
                            type="submit"
                            class="rounded-2xl bg-gradient-to-r from-orange-500 to-red-500 px-6 py-2.5 text-xl font-semibold text-white shadow-lg"
                        >
                            Enregistrer
                        </button>
                    </div>
