...
-->
<template>
    <div class="mt-5 inline-block rounded-2xl border border-blue-100 bg-blue-50 p-4 shadow-sm" style="width: 280px;">
        ...
        <div class="flex items-center justify-between">
            <div class="rounded-xl bg-blue-100 p-2">
                <svg class="h-4 w-4 text-blue-500" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                    <path d="M7 7a5 5 0 0 1 10 0"/><path d="M4 9h16l2 11H2L4 9z"/>
                </svg>
            </div>
            <span class="rounded-full px-2 py-0.5 text-xs font-semibold" :class="diffClass">
                {{ diffLabel }}
            </span>
        </div>

        <p class="mt-3 text-xs font-medium text-blue-700">Évolution du poids</p>

        <!-- Valeurs -->
        <div class="mt-2 flex items-end justify-between">
            <div>
                <p class="text-[10px] text-slate-400">Initial</p>
                <p class="text-xl font-bold text-slate-700">{{ initialWeight ?? '—' }} <span class="text-xs font-normal text-slate-400">kg</span></p>
            </div>
            <div class="text-right">
                <p class="text-[10px] text-slate-400">Actuel</p>
                <p class="text-xl font-bold text-indigo-600">{{ currentWeight ?? '—' }} <span class="text-xs font-normal text-slate-400">kg</span></p>
            </div>
        </div>
