...
import {
    ...
    mapperPatient,
    mapperDetailPatient,
} from "@/components/doctor/utilitairesMedecin.js";
...
const onglets = computed(() => [
    {
        ...
        icon: IconeUtilisateurs,
    },
    {
        key: "invitations",
        label: "Invitations d'ajout",
        count: invitations.value.length,
        icon: IconeAjoutUtilisateur,
    },
]);
...
async function gererInvitation(id, action, onSuccess) {
    ...
    try {
        ...
        setError(
            `Impossible d'${action === "accept" ? "accepter" : "refuser"} cette invitation pour le moment.`,
        );
    } finally {
