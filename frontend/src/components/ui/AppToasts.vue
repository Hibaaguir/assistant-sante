<template>
    <Teleport v-if="!isInlineMainArea" to="body">
        <div
            class="pointer-events-none fixed left-1/2 top-16 z-[1200] flex w-[min(96vw,1180px)] -translate-x-1/2 flex-col gap-2"
        >
                ...
                >
                    <div class="flex items-center gap-3">
                        <span
                            class="shrink-0"
                            :class="
                                STYLES[toast.type]?.text ?? STYLES.info.text
                            "
                            aria-hidden="true"
...
const isInlineMainArea = computed(() =>
    String(route.path ?? "").startsWith("/main"),
);
