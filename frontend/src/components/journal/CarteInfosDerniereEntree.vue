<template>
  <div class="rounded-[28px] border border-[#ddd2ff] bg-gradient-to-br from-[#f5f0ff] via-white to-[#eef4ff] p-8 shadow-sm shadow-violet-200/50 transition hover:-translate-y-0.5 hover:shadow-md">
    ...
    <p class="mt-2 text-sm text-slate-600">Indicateurs complémentaires associés à votre dernière entrée.</p>
...
const INTENSITY = {
  high:  { label: 'Intense',  badge: 'border-rose-300 bg-rose-100 text-rose-700' },
  light: { label: 'Légère',   badge: 'border-sky-300 bg-sky-100 text-sky-700' },
}
const INTENSITY_DEFAULT = { label: 'Modérée', badge: 'border-emerald-300 bg-emerald-100 text-emerald-700' }

const SUGAR = {
  high: { label: 'Élevé',  badge: 'border-rose-300 bg-rose-100 text-rose-700' },
  low:  { label: 'Faible', badge: 'border-emerald-300 bg-emerald-100 text-emerald-700' },
}
const SUGAR_DEFAULT = { label: 'Modéré', badge: 'border-amber-300 bg-amber-100 text-amber-700' }
...
const fmt = {
  ...
  meals: (e) => {
    ...
    const kinds = [...new Set(e.meals.map(m => MEAL_TYPES[m.type] ?? m.type))]
    return `${e.meals.length} repas / ${kinds.join('/')}`
  },

  activity: (e) =>
    e.activityType ? `${e.activityType} ${e.activityDuration} min` : 'Non renseignée',

  tobacco: (e) => {
    if (!e.tobacco) return 'Non'
    const parts = [
      e.tobaccoTypes?.cigarette && e.cigarettesPerDay != null && `Cigarette • ${e.cigarettesPerDay}/j`,
      e.tobaccoTypes?.vape      && e.vapeFrequency            && `Vape • ${e.vapeFrequency} • ${e.vapeLiquidMl} ml`,
    ].filter(Boolean)
