  ...
  <div v-if="open" class="fixed inset-0 z-50 bg-black/35 p-4" @click.self="$emit('close')">
    <div class="mx-auto mt-6 w-full max-w-[540px] rounded-3xl border border-slate-200 bg-gradient-to-br from-white via-[#f9f7ff] to-[#f3f8ff] p-4 shadow-[0_25px_50px_rgba(15,23,42,0.25)] sm:mt-10 sm:p-5">
      ...
      <div class="mb-4 flex items-center justify-between">
        ...
        <button type="button" class="text-slate-500" @click="$emit('close')">×</button>
      </div>
      ...
      <DatePicker v-if="model.type === 'month'" v-model="model.month" type="month" label="Sélectionnez un mois" />
      <DatePicker v-if="model.type === 'date'"  v-model="model.date"  type="date"  label="Sélectionnez une date" />

      <!-- Actions -->
      <div class="mt-4 flex gap-2">
        <button type="button" class="flex-1 rounded-xl bg-gradient-to-r from-[#2563eb] to-[#7c3aed] py-2 text-sm font-semibold text-white" @click="$emit('apply', model)">
          Appliquer
        </button>
        <button type="button" class="rounded-xl bg-slate-100 px-4 py-2 text-sm font-semibold text-slate-500" @click="$emit('reset')">
          Réinitialiser
        </button>
      </div>
