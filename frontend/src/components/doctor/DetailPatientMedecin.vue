<!--
  DetailPatientMedecin.vue
  Détail d'un patient : signes vitaux, analyses, historique traitements, observation générale.
-->
<template>
  <section class="mt-8">
      ...
      <article class="rounded-[12px] border border-[#d4d9e1] bg-[#fcfdff] p-3">
        <div class="flex flex-wrap items-center justify-between gap-2">
          <div class="flex items-center gap-2">
            <p class="text-[15px] font-bold text-[#041c49]">Observation générale du patient</p>
        ...
        <div class="mt-1.5 flex items-center justify-end">
          <p v-if="obsSavedAt" class="text-[11px] text-[#6a7891]">Sauvegardée le {{ obsSavedAt }}</p>
        </div>
        ...
        <div v-if="obsOpen" class="mt-3 space-y-2.5">
          ...
          <div class="flex flex-wrap justify-end gap-2">
            <button type="button" class="btn-outline" @click="effacerObs">Effacer</button>
            <button type="button" class="btn-primary" @click="enregistrerObs">Enregistrer</button>
          </div>
        </div>

        <div v-if="obsMessage" class="mt-2.5 rounded-[12px] border px-3 py-2 text-[12px]"
          :class="obsMessageType === 'success' ? 'border-[#c6ead0] bg-[#f2fcf4] text-[#118445]' : 'border-[#f1d4ae] bg-[#fff8ef] text-[#b46910]'">
          {{ obsMessage }}
        </div>
      </article>
    </section>
    ...
    <nav class="mt-8 rounded-[18px] border border-[#d4d9e1] bg-white p-[10px] shadow-[0_1px_4px_rgba(15,23,42,0.05)]">
      <div class="flex flex-wrap gap-2">
        <button
          ...
          :class="activeTab === tab.key ? 'bg-[#3f49f4] text-white shadow-[0_10px_18px_rgba(63,73,244,0.22)]' : 'text-[#384860]'"
    ...
    <section v-if="activeTab === 'vitals'" class="mt-8 space-y-4">
      <FilterCard title="Filtrer les signes vitaux" subtitle="Affinez l'historique par date et par type de mesure."
        :show-reset="!!(vitalDate || vitalSign !== 'all')" @reset="vitalDate = ''; vitalSign = 'all'">
      ...
      <article v-for="entry in filteredVitals" :key="entry.isoDate || entry.date" class="card">
        <div class="flex items-center gap-2 text-[16px] font-bold text-[#061a45]">
          <IconeCalendrier class="h-[18px] w-[18px]" />{{ entry.date }}
        </div>
        <div class="mt-4 grid gap-4 lg:grid-cols-3">
          <div v-for="card in entry.cards" :key="card.key" class="rounded-[16px] border px-5 py-4" :class="card.class">
      ...
      <EmptyState v-if="!filteredVitals.length" title="Aucun signe vital ne correspond aux filtres." subtitle="Essayez une autre date ou choisissez un autre type de mesure." />
    </section>

    <!-- ── Analyses ────────────────────────────────────────────────────────── -->
    <section v-else-if="activeTab === 'analyses'" class="mt-8 space-y-4">
      <FilterCard title="Filtrer les analyses" subtitle="Affinez les résultats par date et par type d'analyse. Les 7 derniers jours sont affichés par défaut."
        :show-reset="!!(analysisDate || analysisType !== 'all')" @reset="analysisDate = ''; analysisType = 'all'">
      ...
      <article v-for="a in filteredAnalyses" :key="`${a.name}-${a.isoDate}`" class="card px-6 py-5">
        ...
        <div class="mt-4 flex flex-wrap items-center gap-x-6 gap-y-3 text-[15px] text-[#455572]">
          ...
          <span>{{ a.range }}</span>
          <span class="inline-flex items-center gap-2"><IconeCalendrier class="h-[16px] w-[16px]" />{{ a.date }}</span>
        </div>
      </article>

      <EmptyState v-if="!filteredAnalyses.length" title="Aucune analyse ne correspond aux filtres." subtitle="Essayez une autre date ou un autre type d'analyse." />
    </section>
      ...
      <article class="card p-6">
        <div class="flex items-center justify-between gap-3">
          <div>
            ...
            <p class="mt-1 text-[14px] text-[#5b6b84]">Historique réel des traitements pris par le patient, avec 7 derniers jours affichés par défaut.</p>
          </div>
        ...
        <article class="mt-6 rounded-[18px] border border-[#dbe2ec] bg-[#fbfcfe] p-5">
          <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            ...
            <button v-if="treatDate || treatMed !== 'all' || treatStatus !== 'all'" type="button" class="btn-outline" @click="treatDate = ''; treatMed = 'all'; treatStatus = 'all'">
              Réinitialiser
            </button>
          </div>

          <div class="mt-5 grid gap-4 md:grid-cols-3">
            <div><label class="filter-label">Date</label><input v-model="treatDate" type="date" class="input-field" /></div>
            <div>
        ...
        <div v-if="filteredTreatments.length" class="mt-6 space-y-4">
          <article v-for="day in filteredTreatments" :key="day.dateKey" class="rounded-[18px] border border-[#dbe2ec] bg-[#fbfcfe] p-5">
            <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              ...
              <span class="inline-flex h-[34px] items-center rounded-full px-3 text-[12px] font-semibold"
            ...
            <div class="mt-4 grid gap-3 lg:grid-cols-2">
              <article v-for="med in day.meds" :key="`${day.dateKey}-${med.id}`" class="rounded-[16px] border border-[#e2e8f0] bg-white px-4 py-4">
                <div class="flex items-start justify-between gap-3">
                  <div>
                    <p class="text-[15px] font-bold text-[#112652]">{{ med.name }}</p>
                    <p class="mt-1 text-[13px] text-[#63758d]">{{ med.dose }}</p>
                  </div>
                  <span class="inline-flex h-[30px] min-w-[52px] items-center justify-center rounded-full px-3 text-[12px] font-semibold"
                    :class="med.isComplete ? 'bg-[#dff6e7] text-[#15803d]' : 'bg-[#edf2ff] text-[#3257d6]'">
                    {{ med.taken }}/{{ med.total }}
                  </span>
                </div>
                <div class="mt-4 h-[8px] rounded-full bg-[#dfe5ee]">
                  <div class="h-[8px] rounded-full transition-all" :class="med.isComplete ? 'bg-[#16a34a]' : 'bg-[#4f46e5]'" :style="{ width: `${med.progress}%` }" />
                </div>
              </article>
            </div>
          </article>
...
import { IconeFlecheGauche, IconeCalendrier, IconeHorloge, IconeCoeur, IconeLien, IconeOnde } from '@/components/doctor/IconesMedecin.js'
...
// ─── Filtres ──────────────────────────────────────────────────────────────────
const vitalDate    = ref(''); const vitalSign    = ref('all')
const analysisDate = ref(''); const analysisType = ref('all')
const treatDate    = ref(''); const treatMed     = ref('all'); const treatStatus = ref('all')

// ─── Observation ──────────────────────────────────────────────────────────────
const obsText        = ref('')
const obsOpen        = ref(false)
const obsSavedAt     = ref('')
const obsMessage     = ref('')
const obsMessageType = ref('success')

const observationResume = computed(() => {
  const t = obsText.value.trim()
  if (!t) return 'Aucune observation générale pour le moment.'
...
const filteredVitals = computed(() => {
  ...
  return pool
    ...
    .map(e => ({
      ...e,
      cards: VITAL_CARDS
        .filter(c => vitalSign.value === 'all' || c.key === vitalSign.value)
...
const treatMedOptions = computed(() =>
  [...new Set((props.patient?.treatmentHistoryRows ?? [])
...
// ─── Observation API ──────────────────────────────────────────────────────────
async function saveObs(value) {
  try {
    const { data } = await api.put(`/doctor-invitations/patients/${props.patient.id}/observation-generale`, { observation: value })
    const payload  = data?.data?.general_observation ?? {}
    obsText.value    = payload.text ?? (value || '')
    obsSavedAt.value = formatObsDate(payload.updated_at)
    obsMessageType.value = 'success'
    obsMessage.value     = data?.message || (value ? 'Observation générale enregistrée.' : 'Observation générale effacée.')
    obsOpen.value = false
  } catch {
    obsMessageType.value = 'warning'
    obsMessage.value     = "Impossible d'enregistrer l'observation générale pour le moment."
  }
}

function enregistrerObs() {
  const t = obsText.value.trim()
  if (!t) { obsMessageType.value = 'warning'; obsMessage.value = "Écrivez une observation avant de l'enregistrer."; return }
  saveObs(t)
}
...
// ─── Init observation depuis patient ─────────────────────────────────────────
watch(
  () => [props.patient?.id, props.patient?.generalObservation?.updatedAt],
  () => {
    obsMessage.value = ''
    obsText.value    = props.patient?.generalObservation?.text ?? ''
    obsSavedAt.value = formatObsDate(props.patient?.generalObservation?.updatedAt)
    obsOpen.value    = false
  },
  { immediate: true }
)
...
.btn-primary  { @apply inline-flex h-[34px] items-center justify-center rounded-[12px] bg-[#3f49f4] px-3 text-[13px] font-semibold text-white transition hover:bg-[#3140ef]; }
.filter-label { @apply mb-2 block text-[14px] font-semibold text-[#23375f]; }
</style>