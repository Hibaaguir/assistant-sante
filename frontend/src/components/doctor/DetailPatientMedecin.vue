...
<template>
    <section class="mt-8">
        ...
        >
            <article
                class="rounded-[12px] border border-[#d4d9e1] bg-[#fcfdff] p-3"
            >
                <div class="flex flex-wrap items-center justify-between gap-2">
                    <div class="flex items-center gap-2">
                        <p class="text-[15px] font-bold text-[#041c49]">
                            Observation générale du patient
                        </p>
                        <span
                            class="rounded-full bg-[#f3f6fb] px-2 py-0.5 text-[11px] text-[#5c6d89]"
                            >Interne</span
                        >
                    </div>
                    <button
                ...
                <div class="mt-1.5 flex items-center justify-end">
                    <p v-if="obsSavedAt" class="text-[11px] text-[#6a7891]">
                        Sauvegardée le {{ obsSavedAt }}
                    </p>
                </div>
...
async function saveObs(value) {
    try {
        const { data } = await api.put(
            `/doctor-invitations/patients/${props.patient.id}/observation-generale`,
            { observation: value },
        );
        ...
        obsMessage.value =
            data?.message ||
            (value
                ? "Observation générale enregistrée."
                : "Observation générale effacée.");
        obsOpen.value = false;
    } catch {
        obsMessageType.value = "warning";
        obsMessage.value =
            "Impossible d'enregistrer l'observation générale pour le moment.";
    }
}
...
watch(
    ...
    () => {
        ...
        obsSavedAt.value = formatObsDate(
            props.patient?.generalObservation?.updatedAt,
        );
        obsOpen.value = false;
    },
    { immediate: true },
