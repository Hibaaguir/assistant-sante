<template>
  <div>
    <button
      ...
      :class="[error ? 'border-red-300' : 'border-gray-200', disabled ? 'opacity-50 cursor-not-allowed' : '']"
      :disabled="disabled"
      @click="open = !open"
    >
    ...
    <div v-if="open && !disabled" class="rounded-2xl border border-gray-300 bg-white shadow-sm overflow-hidden">
      ...
      <div class="max-h-40 overflow-auto p-2">
        ...
        <p v-if="!filtered.length" class="px-3 py-2 text-sm text-slate-500">Aucun résultat.</p>
      </div>
