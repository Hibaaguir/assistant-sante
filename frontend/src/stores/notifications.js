...
export const useNotificationsStore = defineStore("notifications", () => {
  ...
  function ajouter({ type = "info", title = "", message = "", duration = 3800 } = {}) {
    const id = ++seq;
    items.value.push({
      id,
      type,
  ...
  function erreur(message, title = "Erreur") {
    return ajouter({ type: "error", title, message, duration: 5200 });
  }

  function avertissement(message, title = "Attention") {
    return ajouter({ type: "warning", title, message, duration: 4600 });
  }
