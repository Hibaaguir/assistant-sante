<template>
  <div class="mx-auto max-w-[1320px] p-4 sm:p-6 lg:p-8">
    <section class="relative overflow-hidden rounded-3xl border border-slate-200 bg-gradient-to-br from-[#eef4ff] via-[#f7f4ff] to-[#f4fbf6] p-6 shadow-sm sm:p-8 lg:p-10">
      <div class="pointer-events-none absolute -right-12 -top-12 h-40 w-40 rounded-full bg-[#8b5cf6]/20 blur-2xl"></div>
      <div class="pointer-events-none absolute -bottom-12 left-10 h-36 w-36 rounded-full bg-[#22c55e]/20 blur-2xl"></div>

      <div class="grid items-center gap-8 lg:grid-cols-[1.1fr_0.9fr]">
        <div>
          <p class="inline-flex items-center rounded-full border border-[#c7d9ff] bg-white/70 px-4 py-1.5 text-xs font-semibold text-[#3256c6]">Journal Quotidien HealthFlow</p>
          <h2 class="mt-4 text-4xl font-bold tracking-tight text-slate-900 sm:text-5xl">Suivez votre journee en <span class="bg-gradient-to-r from-[#2563eb] to-[#7c3aed] bg-clip-text text-transparent">toute simplicite</span></h2>
          <p class="mt-4 max-w-xl text-base text-slate-600 sm:text-lg">Une interface structuree pour consigner vos indicateurs quotidiens: sommeil, nutrition, hydratation, activite physique et habitudes de vie.</p>
        ...
        <div class="rounded-3xl border border-white/70 bg-white/80 p-3 shadow-xl shadow-slate-300/40">
          <img
    ...
    <section class="mt-6 grid gap-6 md:grid-cols-2 xl:grid-cols-3">
      <div class="rounded-[28px] border border-[#cfe0ff] bg-gradient-to-br from-[#edf4ff] via-white to-[#f8fbff] p-8 shadow-sm shadow-blue-200/50 transition hover:-translate-y-0.5 hover:shadow-md">
        ...
        <div v-if="latest" class="mt-6 space-y-4 text-base text-slate-700">
        ...
        <div v-else class="mt-6 rounded-xl border border-dashed border-slate-300 bg-slate-50 p-4 text-sm text-slate-500">
          Aucune donnee
        </div>
      </div>
      ...
      <div class="rounded-[28px] border border-[#d9ccff] bg-gradient-to-br from-[#f1eaff] via-[#f8f3ff] to-[#ecf2ff] p-8 shadow-sm shadow-violet-200/50">
        <h3 class="text-3xl font-bold text-slate-900">💡 Conseil du jour</h3>
        <p class="mt-3 text-sm text-slate-600">Maintenez une saisie reguliere de vos donnees pour obtenir un suivi clair, utile et exploitable dans le temps.</p>
        <div class="mt-6 rounded-2xl border border-[#d9e5ff] bg-white/90 p-4">
          <p class="text-sm font-semibold text-slate-700">🎯 Objectif quotidien</p>
          <p class="mt-1 text-xs text-slate-500">Atteindre au moins 2.0 L d'eau et 30 min d'activite legere.</p>
        </div>
      </div>
    </section>
  </div>
