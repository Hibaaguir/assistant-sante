    ...
    >
        <header>
            ...
            <p class="mt-1 text-[13px] text-slate-500">
                Suivez vos indicateurs de santé dans le temps
            </p>
        </header>
...
const libelleBoutonAjout = computed(() =>
...
const formaterLibelle = (iso) =>
    versDate(iso)?.toLocaleDateString("fr-FR", {
        day: "2-digit",
        month: "short",
    }) ?? "";
...
async function chargerDonneesSante() {
    try {
        ...
        diastolicValues.value = normaliserSerie(
            ...
            for (const item of allTreatmentChecks) {
                ...
                if (
                    ...
                    treatmentChecks[item.check_date][
                        construireClePrise(item.medication_key, 1)
                    ] = Boolean(item.taken);
                }
            }
        }
