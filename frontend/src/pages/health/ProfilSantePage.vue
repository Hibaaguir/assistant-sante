        ...
        <div v-else class="grid gap-4 lg:grid-cols-2">
            ...
            >
                ...
                <div v-if="!editing.habits" class="space-y-2">
                    ...
                    <LigneChampSante
                        label="Traitements"
                        :value="treatmentsSummary(profil.traitements)"
                        icon="pill"
                    />
                </div>
            ...
            >
                ...
                <div v-if="!editing.doctor" class="space-y-2">
                    <LigneChampSante
                        label="Consulte médecin"
                        :value="ouiNon(profil.consulte_medecin)"
                        icon="stetho"
                    />
                    <LigneChampSante
                        label="Autorise accès médecin"
                        :value="ouiNon(profil.medecin_peut_consulter)"
                        icon="shield"
                    />
                    <LigneChampSante
                        label="Email médecin"
                        :value="profil.medecin_email || '-'"
                        icon="stetho"
                    />
                </div>
                ...
                <form
                    ...
                    class="space-y-4"
                    novalidate
                    @submit.prevent="enregistrerSection('doctor')"
                >
                    <div>
                        <label
                            class="mb-1 block text-sm font-semibold text-slate-900"
                            >Consulte médecin</label
                        >
                        <select
                            v-model="draft.consulte_medecin"
                            class="h-11 w-full rounded-xl border border-slate-200 bg-slate-100 px-4 text-base"
                            @change="validerEmailMedecin"
                        >
                    ...
                    <div>
                        <label
                            class="mb-1 block text-sm font-semibold text-slate-900"
                            >Autorise accès médecin</label
                        >
                        <select
                            v-model="draft.medecin_peut_consulter"
                            :disabled="!draft.consulte_medecin"
                            class="h-11 w-full rounded-xl border border-slate-200 bg-slate-100 px-4 text-base disabled:opacity-60"
                            @change="validerEmailMedecin"
                        >
                            <option :value="true">Oui</option>
                            <option :value="false">Non</option>
                    ...
                    <div>
                        ...
                        <input
                            ...
                            :disabled="
                                !(
                                    draft.consulte_medecin &&
                                    draft.medecin_peut_consulter
                                )
                            "
...
function appendUniqueCatalogOption(targetRef, value) {
    ...
    const exists = targetRef.value.some(
        (item) =>
            item.localeCompare(normalizedValue, "fr", {
                sensitivity: "base",
            }) === 0,
    );
...
function ensureTreatmentType(type) {
    ...
    if (!treatmentTypes.value.includes(normalizedType)) {
        treatmentTypes.value = [...treatmentTypes.value, normalizedType].sort(
...
async function addCustom(key, value) {
    ...
    if (key === "allergies") {
        ...
        try {
            ...
            notifications.avertissement(
                "Allergie ajoutee au profil local, mais la mise a jour du catalogue partage a echoue.",
            );
        }
    }
    ...
    if (key === "maladies_chroniques") {
        ...
        try {
            ...
            notifications.avertissement(
                "Maladie chronique ajoutee au profil local, mais la mise a jour du catalogue partage a echoue.",
            );
        }
    }
...
async function saveTreatmentDraft() {
    ...
    try {
        await persistTreatmentCatalogEntry(
            nextTreatment.type,
            nextTreatment.name,
        );
    } catch {
...
function syncDraftFromProfil() {
    ...
    draft.maladies_chroniques.forEach((item) =>
...
function construireChargeUtile() {
    ...
    const medecinPeutConsulter = Boolean(
        consulteMedecin && draft.medecin_peut_consulter,
    );
    ...
    return {
        ...
        medecin_email: medecinPeutConsulter
            ? draft.medecin_email || null
            : null,
    };
}
