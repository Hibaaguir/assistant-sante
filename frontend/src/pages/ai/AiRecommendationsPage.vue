        ...
        <div v-else-if="report" class="space-y-6">
            ...
            <div class="relative overflow-hidden rounded-2xl p-6 shadow-sm" :class="riskStyle.bg">
                <div class="flex items-center gap-5">
                    <div class="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl text-3xl shadow-inner" :class="riskStyle.iconBg">
                        {{ riskStyle.icon }}
                    </div>
            ...
            <section v-if="report.alerts?.length">
                <SectionTitle icon="🚨" title="Alertes urgentes" />
                <div class="space-y-3">
                    <div
            ...
            <section v-if="report.global_recommendations?.length">
                <SectionTitle icon="✅" title="Recommandations prioritaires" />
                <div class="space-y-3">
                    <div
            ...
            <section v-if="report.anomalies?.length">
                <SectionTitle icon="⚠️" title="Anomalies détectées" />
                <div class="space-y-2">
                    <div
                    ...
                    >
                        <span class="shrink-0 rounded-full px-2.5 py-0.5 text-xs font-bold uppercase" :class="severityBadge(a.severity)">
                            {{ severityFr(a.severity) }}
                        </span>
                        <p class="text-sm text-slate-700">{{ a.message }}</p>
                    </div>
                </div>
            </section>
            ...
            <section v-if="validPatterns.length">
                <SectionTitle icon="🔗" title="Connexions entre domaines" />
                <div class="space-y-3">
                    <div
            ...
            <section v-if="activeDomains.length">
                <SectionTitle icon="📊" title="Analyse par domaine" />
                    ...
                    >
                        ...
                        <div class="mb-3 flex items-center justify-between">
                            <div class="flex items-center gap-2">
                                <span class="text-base">{{ domainIcon(domain) }}</span>
...
const riskStyle = computed(() => {
    ...
    if (level === "high")   return { bg: "bg-red-50 border border-red-200",     text: "text-red-800",    sub: "text-red-600",    label: "text-red-400",    icon: "🔴", iconBg: "bg-red-100" };
    if (level === "medium") return { bg: "bg-orange-50 border border-orange-200", text: "text-orange-800", sub: "text-orange-600", label: "text-orange-400", icon: "🟠", iconBg: "bg-orange-100" };
    return                         { bg: "bg-green-50 border border-green-200",   text: "text-green-800",  sub: "text-green-600",  label: "text-green-400",  icon: "🟢", iconBg: "bg-green-100" };
});
...
const DOMAIN_ICONS = {
    ...
    alcohol: "🍷", vital_signs: "❤️", lab_results: "🔬", treatments: "💊",
};
