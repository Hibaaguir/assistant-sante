<template>
  <div class="min-h-screen bg-[#edf2f9] py-4">
    <div class="mx-auto max-w-[1160px] px-3 sm:px-4 lg:px-6">
      ...
      <header class="rounded-[16px] border border-[#c8d4e4] bg-[#edf2f9] p-4 shadow-[0_1px_4px_rgba(15,23,42,0.05)] sm:p-4">
        <div class="flex flex-wrap items-start justify-between gap-3">
          <div>
            <h1 class="bg-gradient-to-r from-[#3d6df2] via-[#6f49e8] to-[#b832df] bg-clip-text text-[44px] font-bold leading-none tracking-[-0.015em] text-transparent sm:text-[47px]">
              Mes Tâches Bien-être
            </h1>
            <p class="mt-1.5 text-[15px] font-normal text-slate-600">Gérez vos objectifs quotidiens de bien-être</p>
          </div>

          <div class="min-w-[146px] rounded-[18px] border border-[#c9d8ea] bg-white/95 px-3.5 py-2.5 shadow-[0_4px_12px_rgba(15,23,42,0.08)]">
            <div class="flex items-center gap-2">
              <span class="inline-flex h-8 w-8 items-center justify-center rounded-xl bg-[#eef4ff] text-[#2563eb]">
                <svg viewBox="0 0 24 24" class="h-4.5 w-4.5" fill="none" stroke="currentColor" stroke-width="2.2">
                  <path d="M9 11l3 3L22 4" />
                  <path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11" />
                </svg>
              </span>
              <div>
                <p class="text-[34px] font-extrabold leading-none tracking-[-0.02em] text-slate-900">{{ totalTerminees }}/{{ totalTaches }}</p>
                <p class="mt-0.5 text-[11px] font-medium leading-none text-slate-500">Tâches complétées</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="mt-3.5 flex flex-wrap items-center gap-2">
          ...
          <div class="flex flex-wrap items-center gap-2">
            <button
              ...
              :class="categorieActive === onglet.valeur
      ...
      <section class="mt-5 grid gap-5 lg:grid-cols-[286px_1fr]">
        <aside class="space-y-5">
          <div class="min-h-[232px] rounded-[24px] border border-[#a9c8ff] bg-gradient-to-br from-[#dce8ff] to-[#eeddf8] p-5 shadow-[0_8px_16px_rgba(15,23,42,0.10)]">
            <div class="flex items-center gap-3">
              <span class="inline-flex h-10 w-10 items-center justify-center rounded-2xl bg-gradient-to-br from-[#60a5fa] to-[#8b5cf6] text-white">
              ...
              <h3 class="text-[22px] font-bold leading-tight text-slate-800">Message du jour</h3>
            </div>

            <p class="mt-3.5 text-[16px] font-medium leading-relaxed text-slate-700">
              Vous avez commencé, c'est déjà une victoire ! Continuez d'avancer. ✨
            </p>

            <div class="my-4 h-px bg-[#9cc3ff]/75"></div>

            <p class="text-[12px] italic leading-relaxed text-slate-600">
              "Prenez soin de votre corps, c'est le seul endroit où vous devez vivre."
            </p>
          </div>

          <div class="rounded-[24px] border border-slate-300 bg-white p-5 shadow-[0_6px_14px_rgba(15,23,42,0.08)]">
            <h3 class="flex items-center gap-2 text-[26px] font-bold text-slate-800">
              <span class="text-[#9333ea]">◈</span>
              Catégories
            </h3>

            <ul class="mt-4 space-y-3">
              <li v-for="categorie in categoriesAffichees" :key="categorie.cle" class="flex items-center justify-between text-[15px]">
                <div class="flex items-center gap-3 text-slate-600">
                  <span class="h-3.5 w-3.5 rounded-full" :style="{ backgroundColor: categorie.couleur }"></span>
                  <span>{{ categorie.libelle }}</span>
                </div>
                <span class="text-[15px] font-semibold text-slate-700">{{ compteParCategorie[categorie.cle] || 0 }}</span>
              </li>
            </ul>
          </div>
        </aside>
        ...
        <div class="space-y-4">
          <div class="rounded-[22px] border border-slate-300 bg-white p-4 shadow-[0_8px_18px_rgba(15,23,42,0.08)]">
            <div class="flex flex-wrap items-center gap-3">
              ...
              >
                {{ idTacheEnEdition ? 'Mettre à jour' : '+ Ajouter' }}
              </button>
            </div>
          </div>

          <div v-if="chargementInitial" class="rounded-[20px] border border-slate-300 bg-white p-6 text-base text-slate-500">
            Chargement des tâches...
          </div>

          <TransitionGroup v-else name="liste-taches" tag="div" class="space-y-3.5">
            <article
            ...
            >
              <div class="flex items-center gap-4">
                ...
                >
                  <svg viewBox="0 0 24 24" class="h-5 w-5" fill="none" stroke="currentColor" stroke-width="2.8">
                    <path d="M5 13l4 4L19 7" />
                  </svg>
                </button>

                <div class="min-w-0 flex-1">
                  <p class="truncate text-[22px] font-semibold leading-[1.22] tracking-[-0.01em] text-slate-800 sm:text-[23px]" :class="tache.est_complete ? 'line-through text-slate-500' : ''">
                    {{ tache.titre }}
                  </p>

                  <div class="mt-2 flex flex-wrap items-center gap-2">
                    <span class="rounded-full px-2.5 py-0.5 text-[12px] font-semibold text-white" :style="{ backgroundColor: couleurCategorie(tache.categorie) }">
                      {{ libelleCategorie(tache.categorie) }}
                    </span>

                    <span v-if="tache.date_echeance" class="text-[13px] font-medium text-slate-500">📅 {{ tache.date_echeance }}</span>
                  </div>
                </div>

                <div class="flex items-center gap-2 opacity-100 transition lg:opacity-0 lg:group-hover:opacity-100">
                  <button
                    type="button"
            ...
            <div v-if="!tachesFiltrees.length" key="vide" class="rounded-[20px] border border-dashed border-slate-300 bg-white px-5 py-10 text-center text-[18px] text-slate-500">
...
const tachesFiltrees = computed(() => {
  ...
  return toutesLesTaches.value.filter((tache) => {
    const parCategorie = categorieActive.value === "toutes" || tache.categorie === categorieActive.value;
    const parRecherche = !filtreTexte || String(tache.titre || "").toLowerCase().includes(filtreTexte);
...
async function soumettreTache() {
  ...
  if (titre.length < 2) {
    notifications.avertissement("Le titre doit contenir au moins 2 caractères.");
    return;
  }
  ...
  const payload = {
    ...
    categorie:
      formulaireTache.categorie ||
      (categorieActive.value !== "toutes" ? categorieActive.value : "bien-etre"),
  ...
  try {
    ...
    if (erreur?.response?.status === 422) {
      notifications.avertissement("Veuillez vérifier les champs de la tâche.");
      return;
    }
...
async function supprimerTache(idTache) {
  try {
    await storeTaches.supprimer(idTache);
    notifications.actionSupprimee("Tâche supprimée avec succès.");

    if (idTacheEnEdition.value === idTache) {
      reinitialiserFormulaire();
    }
  } catch {
    notifications.erreur("Impossible de supprimer la tâche.");
  }
}
