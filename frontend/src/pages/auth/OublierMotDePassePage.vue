<template>
  <div class="flex min-h-screen items-center justify-center bg-gray-50 px-4 py-12 sm:px-6 lg:px-8">
    <div class="w-full max-w-lg space-y-6">
      ...
      <!-- Content -->
      <div>
        <h2 class="text-xl font-semibold text-gray-900">Mot de passe oublié ?</h2>
      ...
      <form class="space-y-4" @submit.prevent="demanderReinit">
        <div>
          ...
          />
          <p v-if="erreurs.email" class="mt-2 text-xs text-rose-600">{{ erreurs.email }}</p>
        </div>
      ...
      <Transition name="fade">
        <div v-if="messageSucces" class="rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-700">
          {{ messageSucces }}
        </div>
      </Transition>
      ...
      <div class="rounded-xl bg-blue-50 p-4 text-sm text-blue-800">
        ...
        <p class="mt-1">Vérifiez votre dossier spam si vous ne recevez pas l'email de réinitialisation.</p>
      </div>
    </div>
  </div>
</template>
...
async function demanderReinit() {
  ...
  try {
    ...
    if (response?.data) {
      messageSucces.value = 'Un lien de réinitialisation a été envoyé à votre email.'
      notifications.succes('Vérifiez votre boîte mail pour le lien de réinitialisation.')
      formulaire.email = ''
